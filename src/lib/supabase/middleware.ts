...
export async function updateSession(request: NextRequest) {
    ...
    let { data: { user }, error } = await supabase.auth.getUser();
        ...
        // 🛡️ [Security] Absolute Session Timeout (24 Hours)
        ...
        if (user && user.last_sign_in_at && !isSigninPage) {
            ...
            const oneDay = 24 * 60 * 60 * 1000; // 24시간으로 연장

            if (now - lastSignIn > oneDay) {
                console.warn(`[Middleware] Session Expired (Absolute Timeout > 24h): ${user.email}`);
                // 세션 만료 시 리프레시 토큰으로 갱신 시도하도록 유도 (강제 로그아웃 대신)
                // 하지만 보안을 위해 24시간 경과 시에는 재로그인을 요구하는 것이 표준임.
